...
const addSaleToClient = async (clientId: IdType, saleId: IdType, totalSale: number, session: ClientSession) => {
    try {
       ...
       if(client.sales && client.balance !== undefined) {
            const saleForClient = {
                ...sale.toObject(),
